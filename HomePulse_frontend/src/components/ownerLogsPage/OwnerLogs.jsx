...
const OwnerLogs = ({ teamID, access_token }) => {
    ...
    const fetchLogsHistory = () => {
        axios.get(
            `${VITE_BACKEND_HOST}/api/1.0/groups/getTeamLogsHistory?teamID=${teamID}`,
            {
                headers: {
    ...
    return (
        <div className='flex flex-col gap-10 h-full overflow-y-auto'>
            ...
            {/* {
                logsHistory.map((log, idx) => (
                    <OwnerLogCompo key={idx} logInfo={log} />
                ))
            } */}
            {/* {
